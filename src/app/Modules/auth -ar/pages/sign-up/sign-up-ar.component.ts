...
@Component({
  ...
  templateUrl: './sign-up-ar.component.html',
  encapsulation: ViewEncapsulation.None,
  providers: [MessageService],
})
export class SignUpArComponent {
  form = new FormGroup({
  ...
  private isUserExists(mobile: string | null) {
  ...
  signUp() {
    const mobileValue = this.form.controls.mobile.value;
    if (this.isUserExists(mobileValue)) {
      this.already();
      return;
    }
    if (this.form.valid) {
  ...
  invalid() {
    this.messageService.add({
      severity: 'error',
      summary: 'خطأ',
      detail: ' البيانات غير مكتمله، يرجى التحقق من بيانات الإدخال الخاصة بك',
      life: 3000,
    });
  }
  already() {
    this.messageService.add({
      severity: 'error',
      summary: 'خطأ',
      detail: 'لديك حساب بهذا الرقم بالفعل',
      life: 3000,
    });
  }
}
